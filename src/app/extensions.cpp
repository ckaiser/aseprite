...
public:
    ...
    : m_definition(formatDefinition)
  {
    ...
    // A+
    // dd the load/save support flags since we don't expose those to Lua
...
protected:
  ...
  bool onLoad(FileOp* fop) override
  {
    ...
    // TODO: Any way we could clean up any sprites/documents created inside this function call if we
    // fail? Crude solution would just be to get the current list and compare
    ...
    // The hacky way of making the script-created sprite a "new" one without the undo history
    // that the script generates by using the lua Sprite class.
    // TODO: Provide the scripts a "raw" sprite class?
    auto* doc = static_cast<Doc*>(sprite->document());
    Doc* docCopy = doc->duplicate(DuplicateExactCopy);
    doc->close();
    delete doc;

    docCopy->setFilename(fop->filename());
    docCopy->markAsSaved();
    docCopy->setContext(fop->context());
    return true;
  };

  bool onPostLoad(FileOp*) override
  {
    // TODO: Do we need to expose this to scripts?
    return true;
  }
