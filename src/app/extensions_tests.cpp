...
constexpr const auto* kExtensionStartingPrefLuaData = R"(
  return {starting_pref=1234,string="hello",bone=true,btwo=false,table={one=1,two=2},["ünicode"]="unicode"}
)";
...
TEST(Extensions, Basic)
{
  ...
  for (auto* ext : extensions) {
    ...
    testExt = ext;
    EXPECT_EQ(base::get_absolute_path(ext->path()), base::get_absolute_path(kExtensionFolder));
  ...
  EXPECT_EQ(extensions.palettePath("palette1"), "_extensions/_extension/./palette.gpl");

  EXPECT_EQ(extensions.themePath("theme0"), "");
  EXPECT_EQ(extensions.themePath("theme1"), "_extensions/_extension/.");

  EXPECT_EQ(extensions.languagePath("nolang"), "");
  EXPECT_EQ(extensions.languagePath("klin1234"), "_extensions/_extension/./klin1234.ini");
